defmodule Azurex.Authorization.ServicePrincipal do
  ...
  """
  @spec add_bearer_token(Req.Request.t(), binary(), binary(), binary()) ::
          Req.Request.t()
  def add_bearer_token(%Req.Request{} = request, client_id, client_secret, tenant_id) do
    bearer_token = fetch_bearer_token_cached(client_id, client_secret, tenant_id)
    Req.merge(request, headers: [{"Authorization", "Bearer #{bearer_token}"}])
  end
  ...
  defp fetch_bearer_token(client_id, client_secret, tenant_id) do
    ...
    case response do
      {:ok, %{status: 200, body: body}} ->
        ...
        {:ok, Map.fetch!(parsed, "access_token")}

      {:ok, %{status: sc, body: body}} ->
        Logger.error("Failed to fetch bearer token. Reason: #{sc}: #{inspect(body)}")
        :error

      {:error, reason} ->
        Logger.error("Failed to fetch bearer token. Reason: #{inspect(reason)}")
        :error
    end
  end
end
